...
var cheerioHtmlUrl = function(url, doChecksCallback) {
    var buffer = new Buffer(8);
    var saveBuffer = function(result, response) {
        if (result instanceof Error) {
            console.error('Error: ' + util.format(response.message));
        } else {
            buffer = new Buffer(result);
            return doChecksCallback(cheerio.load(buffer));
        }
    }
    rest.get(url).on('complete', saveBuffer);
};
...
var checkHtmlUrl = function(html, checksfile) {
    ...
    var result = cheerioHtmlUrl(html, doChecks);
